...
## 새롭게 추가된 에피소드 : 장애물 2개 사이를 지나는 경로
    ...
    # =========================
    def _inside_map_margin_xy(self, E: float, N: float, margin: float = 5.0) -> bool:
        return (
                margin <= E <= (self.global_map_size - margin)
                and margin <= N <= (self.global_map_size - margin)
        )
    ...
    def _build_global_dubins_path(self):
        ...
        try:
            ...
            if len(path_x) > 0:
                ...
                margin = 5.0  # grid 안쪽 여유
                clipped_x = []
                clipped_y = []

                for i in range(1, num_points + 1):
                    dist = i * step_size
                    cand_x = last_x + dist * math.cos(last_yaw)
                    cand_y = last_y + dist * math.sin(last_yaw)

                    if not self._inside_map_margin_xy(cand_x, cand_y, margin=margin):
                        break

                    clipped_x.append(cand_x)
                    clipped_y.append(cand_y)

                path_x.extend(clipped_x)
                path_y.extend(clipped_y)
