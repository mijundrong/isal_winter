        ...
        if obs_spec is not None:
            ...
            for i in range(obs_arr.shape[0]):
                ...
                if not self._goal_clearance_ok((E, N)):
                    ...
                    raise ValueError(
                        f"Obstacle center must be at least {self.goal_obstacle_min_dist:.1f} m from goal_pos; "
...
## 새롭게 추가된 에피소드 : 장애물 2개 사이를 지나는 경로
    ...
    def _goal_clearance_ok(self, center_xy, min_dist: Optional[float] = None) -> bool:
        ...
        goal_xy = np.asarray(self.goal_pos[:2], dtype=float)
    ...
    def generate_obstacles(
        ...
        # 실제 goal_pos 기준 장애물 중심 최소 이격
        Eg_goal, Ng_goal = float(self.goal_pos[0]), float(self.goal_pos[1])
    ...
    def render(self, fps=30):
        ...
        self.draw_x((gx, gy), size=16, color=self.COLOR_END_X, width=3)
