...
class SimpleMazeGrid(gym.Env):
    ...
    def __init__(
        ...
        enable_two_obstacle_corridor: bool = False,
        corridor_episode_interval: int = 5,
        corridor_path_clearance: Tuple[float, float] = (1.0, 2.0),
        corridor_sample_ratio: Tuple[float, float] = (0.35, 0.65),
        corridor_anchor_jitter: float = 8.0,
        path_obstacle_clearance: Tuple[float, float] = (0.5, 2.0),
        path_obstacle_sample_ratio: Tuple[float, float] = (0.20, 0.80),
        path_obstacle_anchor_jitter: float = 10.0,
        goal_obstacle_min_dist: float = 50.0,
        ...
        self.radius = 150
        self.agent_radius = 0.0  # point-mass 가정
        self.safety_zone = float(safety_zone)
        self.hard_zone = float(hard_zone)

        # scenario / curriculum
        self.enable_two_obstacle_corridor = bool(enable_two_obstacle_corridor)
        self.corridor_episode_interval = max(1, int(corridor_episode_interval))
        self.corridor_path_clearance = (
            float(corridor_path_clearance[0]),
            float(corridor_path_clearance[1]),
        )
        self.corridor_sample_ratio = (
            float(corridor_sample_ratio[0]),
            float(corridor_sample_ratio[1]),
        )
        self.corridor_anchor_jitter = float(corridor_anchor_jitter)
        self.path_obstacle_clearance = (
            float(path_obstacle_clearance[0]),
            float(path_obstacle_clearance[1]),
        )
        self.path_obstacle_sample_ratio = (
            float(path_obstacle_sample_ratio[0]),
            float(path_obstacle_sample_ratio[1]),
        )
        self.path_obstacle_anchor_jitter = float(path_obstacle_anchor_jitter)
        self.goal_obstacle_min_dist = float(goal_obstacle_min_dist)
        self.episode_counter = 0
        self.current_episode_mode = "default"

        # render alpha
        self.render_hard_zone_alpha = 72
        self.render_safe_zone_alpha = 28
    ...
    def _reset_core(self, random_seed=None):
        ...
        self.visited_path = [self.player_pos.copy()]
    ...
    def _reset_core_spec(self, random_seed=None):
        ...
        if obs_spec is not None:
            ...
            for i in range(obs_arr.shape[0]):
                ...
                if not self._goal_clearance_ok((E, N)):
                    dist_goal = float(np.linalg.norm(np.array([E, N]) - np.asarray(self.goal_pos[:2], dtype=float)))
                    raise ValueError(
                        f"Obstacle center must be at least {self.goal_obstacle_min_dist:.1f} m from the terminal end point; "
                        f"got {dist_goal:.2f} m for obstacle ({E:.2f}, {N:.2f}, r={r:.2f})."
                    )
...
## 새롭게 추가된 에피소드 : 장애물 2개 사이를 지나는 경로
    def _is_corridor_episode(self) -> bool:
        return (
            self.enable_two_obstacle_corridor
            and self.spec is None
            and self.corridor_episode_interval > 0
            and (self.episode_counter % self.corridor_episode_interval == 0)
        )

    def _inside_map_with_margin(self, E: float, N: float, margin: float) -> bool:
        return (
            margin <= E <= (self.global_map_size - margin)
            and margin <= N <= (self.global_map_size - margin)
        )

    def _goal_clearance_reference_xy(self):
        target_xy, _ = self._target_goal_state()
        return np.asarray(target_xy[:2], dtype=float)

    def _goal_clearance_ok(self, center_xy, min_dist: Optional[float] = None) -> bool:
        if min_dist is None:
            min_dist = self.goal_obstacle_min_dist
        goal_xy = self._goal_clearance_reference_xy()
        center_xy = np.asarray(center_xy[:2], dtype=float)
        return float(np.linalg.norm(center_xy - goal_xy)) >= float(min_dist)

    def _path_tangent_normal(self, idx: int):
        if self.global_path is None or self.global_path.shape[0] < 3:
            return None, None

        path = np.asarray(self.global_path[:, :2], dtype=float)
        i0 = max(0, idx - 5)
        i1 = min(len(path) - 1, idx + 5)
    ...
    def _path_min_distance_to_circle(self, center_xy, radius_eff: float) -> float:
        if self.global_path is None or self.global_path.shape[0] == 0:
            return float("inf")
        path = np.asarray(self.global_path[:, :2], dtype=float)
        d = np.hypot(path[:, 0] - center_xy[0], path[:, 1] - center_xy[1])
        return float(np.min(d) - radius_eff)

    def _generate_two_obstacle_corridor(self, random_seed=None, max_attempts=2000) -> bool:
        if self.global_path is None or self.global_path.shape[0] < 20:
            return False
        ...
        low_ratio, high_ratio = self.corridor_sample_ratio
        low_idx = max(5, int(low_ratio * (len(path) - 1)))
        high_idx = min(len(path) - 6, int(high_ratio * (len(path) - 1)))
        if high_idx <= low_idx:
            return False

        target_xy, _ = self._target_goal_state()
        start_xy = np.asarray(self.initial_player_pos[:2], dtype=float)
        goal_xy = np.asarray(target_xy, dtype=float)

        clearance_lo = min(self.corridor_path_clearance)
        clearance_hi = max(self.corridor_path_clearance)

        for _ in range(max_attempts):
            idx = random.randint(low_idx, high_idx)
            anchor = path[idx].copy()

            tangent, normal = self._path_tangent_normal(idx)
            if tangent is None:
                continue

            # path를 따라 약간 앞뒤로 흔들어줌
            along_shift = random.uniform(-self.corridor_anchor_jitter, self.corridor_anchor_jitter)
            anchor = anchor + along_shift * tangent

            r1 = random.uniform(self.obstacle_min_r, self.obstacle_max_r)
            r2 = random.uniform(self.obstacle_min_r, self.obstacle_max_r)
            path_clearance = random.uniform(clearance_lo, clearance_hi)

            # hard-zone 내부로 path가 들어가지 않도록 중심 offset 계산
            off1 = r1 + self.hard_zone + path_clearance
            off2 = r2 + self.hard_zone + path_clearance

            c1 = anchor + off1 * normal
            c2 = anchor - off2 * normal

            margin1 = r1 + self.safety_zone + 1.0
            margin2 = r2 + self.safety_zone + 1.0

            if not self._inside_map_with_margin(float(c1[0]), float(c1[1]), margin1):
                continue
            if not self._inside_map_with_margin(float(c2[0]), float(c2[1]), margin2):
                continue

            eff1 = r1 + self.hard_zone + self.agent_radius
            eff2 = r2 + self.hard_zone + self.agent_radius

            # start / goal은 hard-zone 밖
            if np.linalg.norm(c1 - start_xy) <= eff1 or np.linalg.norm(c2 - start_xy) <= eff2:
                continue
            if np.linalg.norm(c1 - goal_xy) <= eff1 or np.linalg.norm(c2 - goal_xy) <= eff2:
                continue

            # goal_pos 주변 50 m 이내에는 장애물 중심 배치 금지
            if not self._goal_clearance_ok(c1) or not self._goal_clearance_ok(c2):
                continue

            # 전역 path가 hard-zone 안으로 들어가면 reject
            if self._path_min_distance_to_circle(c1, eff1) <= 0.35:
                continue
            if self._path_min_distance_to_circle(c2, eff2) <= 0.35:
                continue

            # hard-zone끼리 겹치지 않게
            if np.linalg.norm(c1 - c2) <= (r1 + self.hard_zone) + (r2 + self.hard_zone) + 0.25:
                continue

            self.obstacles = [
                (float(c1[0]), float(c1[1]), float(r1)),
                (float(c2[0]), float(c2[1]), float(r2)),
            ]
            self.current_episode_mode = "corridor-2obs"
            return True

        return False

    def _generate_path_constrained_obstacles(self, count: int, random_seed=None, max_attempts=5000) -> int:
        if count <= 0:
            return 0
        if self.global_path is None or self.global_path.shape[0] < 20:
            return 0
        ...
        path = np.asarray(self.global_path[:, :2], dtype=float)
        low_ratio, high_ratio = self.path_obstacle_sample_ratio
        low_idx = max(5, int(low_ratio * (len(path) - 1)))
        high_idx = min(len(path) - 6, int(high_ratio * (len(path) - 1)))
        if high_idx <= low_idx:
            return 0
        ...
        start_xy = np.asarray(self.initial_player_pos[:2], dtype=float)
        goal_xy = self._goal_clearance_reference_xy()
        clearance_lo = min(self.path_obstacle_clearance)
        clearance_hi = max(self.path_obstacle_clearance)
        ...
        for _ in range(count):
            ...
            for _ in range(max_attempts):
                ...
                along_shift = random.uniform(-self.path_obstacle_anchor_jitter, self.path_obstacle_anchor_jitter)
                anchor = anchor + along_shift * tangent

                r = random.uniform(self.obstacle_min_r, self.obstacle_max_r)
                path_clearance = random.uniform(clearance_lo, clearance_hi)
                side = random.choice([-1.0, 1.0])
                off = r + self.hard_zone + path_clearance
                center = anchor + side * off * normal
                cx, cy = float(center[0]), float(center[1])

                margin = r + self.safety_zone + 1.0
                if not self._inside_map_with_margin(cx, cy, margin):
                    continue

                eff = r + self.hard_zone + self.agent_radius
                if np.linalg.norm(center - start_xy) <= eff:
                    continue
                if np.linalg.norm(center - goal_xy) < self.goal_obstacle_min_dist:
                    continue
                if self._path_min_distance_to_circle(center, eff) <= 0.35:
                    continue
                if any(
                    np.linalg.norm(center - np.asarray([oE, oN], dtype=float)) < ((r + self.hard_zone) + (oR + self.hard_zone) + 0.25)
                    for (oE, oN, oR) in self.obstacles
                ):
                    continue

                self.obstacles.append((cx, cy, float(r)))
                placed = True
                placed_count += 1
                break
            if not placed:
    ...
    def generate_obstacles(
        ...
        """
        기본 에피소드에서는 기존 랜덤 장애물 생성.
        매 N번째 에피소드에서는 path 기준 2-obstacle corridor 생성.
        """
        self.obstacles = []

        if self.obstacle_count <= 0 and not self._is_corridor_episode():
            return

        if self._is_corridor_episode():
            if self._generate_two_obstacle_corridor(random_seed=random_seed, max_attempts=max_attempts):
                return
            self.current_episode_mode = "default-fallback"
        ...
        # 실제 종료 판정 지점(path_end_state 우선) 기준 장애물 중심 최소 이격
        target_xy, _ = self._target_goal_state()
        Eg_goal, Ng_goal = float(target_xy[0]), float(target_xy[1])
            ...
            if d_mid > 1e-6:
                ...
                for _ in range(max_attempts):
                    ...
                    if dist_end < min_sep_with_hardzone(r0_cap):
                        continue
                    if math.hypot(mE_off - Eg_goal, mN_off - Ng_goal) < self.goal_obstacle_min_dist:
                        continue
        ...
        # ---------------------------------------------------------------------
        # 2) remaining obstacles: global path 근처에만 배치
        #    end point(실제 terminal end)와는 최소 50 m 이상 이격
        # ---------------------------------------------------------------------
        remaining = max(0, self.obstacle_count - len(self.obstacles))
        placed_count = self._generate_path_constrained_obstacles(
            remaining,
            random_seed=random_seed,
            max_attempts=max_attempts,
        )

        # path-constrained 배치가 일부 실패하면 마지막으로만 fallback
        ...
        for _ in range(remaining):
            ...
            for _ in range(max_attempts):
                ...
                if math.hypot(E - Eg_end, N - Ng_end) < min_sep_with_hardzone(r):
                    continue
                if math.hypot(E - Eg_goal, N - Ng_goal) < self.goal_obstacle_min_dist:
                    continue
                if any(
    ...
    def _init_render(self):
        ...
        self.global_screen_width = 1100  # 1000
        self.global_screen_height = 1100 # 1000
        self.info_width = 260 # 220
        self.local_screen_width = 650 # 500
        self.local_screen_height = 650 #500
        ...
        self.font = pygame.font.Font(None, 44)  #36
        self.small_font = pygame.font.Font(None, 28)  #20
        ...
        self.COLOR_SAFE_ZONE = (255, 140, 0)
        self.COLOR_START_X = (20, 90, 255)
        self.COLOR_END_X = (20, 170, 20)
    ...
    def draw_obstacles_on_global(self, cell_size):
        ...
        for (oE, oN, oR) in self.obstacles:
            ...
            local_xy = (sx - self.rect_global.x, sy - self.rect_global.y)

            obs_px = max(2, int(round(oR * cell_size)))
            hard_px = max(2, int(round((oR + self.hard_zone) * cell_size)))
            safe_px = max(2, int(round((oR + self.safety_zone) * cell_size)))

            # safety annulus
            if self.safety_zone > self.hard_zone:
                pygame.draw.circle(
                    overlay,
                    (*self.COLOR_SAFE_ZONE, self.render_safe_zone_alpha),
                    local_xy,
                    safe_px,
                    0,
                )
            ...
            pygame.draw.circle(
                overlay,
                (*self.COLOR_HARD_ZONE, self.render_hard_zone_alpha),
                local_xy,
                hard_px,
                0,
            )
        ...
        for (oE, oN, oR) in self.obstacles:
            ...
            pygame.draw.circle(self.screen, self.COLOR_OBS_EDGE, (sx, sy), obs_px, 3)
            pygame.draw.circle(self.screen, self.COLOR_HARD_ZONE, (sx, sy), hard_px, 4)

            if self.safety_zone > self.hard_zone:
                pygame.draw.circle(self.screen, self.COLOR_SAFE_ZONE, (sx, sy), safe_px, 2)
    ...
    def render(self, fps=30):
        ...
        end_xy, _ = self._target_goal_state()
        end_x, end_y = self.world_to_screen(float(end_xy[0]), float(end_xy[1]), self.rect_global, cell_size)
        ...
        if isinstance(self.global_path, np.ndarray) and self.global_path.shape[0] >= 2:
            ...
            pygame.draw.lines(self.screen, self.COLOR_DUBINS, False, dubins_pts, 4)
        ...
        # start / end markers (항상 obstacle 위에 보이도록 마지막에 그림)
        sx0, sy0 = self.world_to_screen(float(self.initial_player_pos[0]), float(self.initial_player_pos[1]), self.rect_global, cell_size)
        self.draw_x((sx0, sy0), size=16, color=self.COLOR_START_X, width=3)

        self.draw_x((end_x, end_y), size=16, color=self.COLOR_END_X, width=3)
        ...
        self.screen.blit(self.small_font.render(f"Episode: {self.episode_counter}", True, (0, 0, 0)), (info_x, 205))
        self.screen.blit(self.small_font.render(f"Mode: {self.current_episode_mode}", True, (0, 0, 0)), (info_x, 230))
        self.screen.blit(self.small_font.render("Blue X: start", True, self.COLOR_START_X), (info_x, 260))
        self.screen.blit(self.small_font.render("Green X: terminal end", True, self.COLOR_END_X), (info_x, 285))
        self.screen.blit(self.small_font.render("Green dot: goal pose", True, self.COLOR_GOAL), (info_x, 310))
