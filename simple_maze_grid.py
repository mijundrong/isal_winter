        ...
        for (oE, oN, oR) in self.obstacles:
            ...
            for r in range(L):
                ...
                for c in range(L):
                    ymin = y_lefts[c]
                    ymax = y_rights[c]
                    if yb_o < (ymin - oR) or yb_o > (max(ymax, ymin) + (ymax-ymin)):
                        pass
    ...
    def visualize_result(env, traj_record):
        ...
        if wps is not None:
            ...
            for i in range(len(wps_arr)):
                if i == len(wps_arr) - 1: continue
                plt.text(wps_x[i] + 1.5, wps_y[i] + 1.5, f"W{i}", fontsize=FS_TICK, color='red')
        ...
        if env.global_path is not None and len(env.global_path) > 0:
            plt.plot(env.global_path[:, 0] - offset, env.global_path[:, 1] - offset,
                     color='cyan', linewidth=3, linestyle='--', label="Global Path")
        ...
        plt.ylim(0, 220) # Scale adjusted for 1500 map
        plt.xlabel("Time [s]", fontsize=FS_LABEL)
        ...
        bound_1m = float(np.mean(err_data < 5.0) * 100.0) # Adjusted bound to 5m for larger scale
        bound_3m = float(np.mean(err_data < 10.0) * 100.0) # Adjusted bound to 10m
...
if __name__ == "__main__":
    ...
    # Scenario 0: 기본 (Original)
    spec = (
        [750, 750, 0],   
        [[1050, 1196.25, 40]],
    )
    waypoints = [
        [750, 750], [1200, 750], [1350, 1050], [750, 1350], [750, 750]
    ]

    # Scenario 1: 15배 스케일 확장형
    spec1 = (
        [750, 750, atan2(-18, 20)],  
        [[1365, 480, 40]],   
    )
    waypoints1 = [
        [750, 750], [1050, 480], [1050, 225], [1350, 225], [1350, 750], [750, 750]
    ]

    # Scenario 2: 중앙을 기준으로 한 거대하고 완벽한 단일 원(Circle) 서킷
    spec2 = (
        [250, 750, radians(90)],  # 9시 방향(맨 왼쪽)에서 시계 방향(북쪽)으로 출발
        [
            # 원 궤적을 도는 중간에 만나는 장애물들 (회피 성능 테스트용)
            [1270, 750, 50],   # 3시 방향 길목 장애물
        ],
    )
    ...
    waypoints2 = [
        ...
        [267, 621],     # W46
        [254, 685],     # W47
        [250, 750],     # W48: 출발 지점으로 완벽히 복귀!
    ]
    
    waypoints3 = [
        [500, 500],    # W0: 시작점 (좌측 하단)
        
        # --- 1. 좌측 요동치는 구간 (빨간 선의 좌측 S자 굴곡) ---
        [430, 650],    # W1: 바깥쪽(좌측)으로 크게 불룩
        [630, 800],    # W2: 안쪽(우측)으로 쏙 들어감
        [500, 950],    # W3: 다시 바깥쪽(좌측)으로 불룩
        [700, 1100],   # W4: 상단 최고점 (크게 돎)
        
        # --- 2. 장애물 우측 통과 구간 ---
        [1070, 1025],   # W5: 장애물(750, 1025)의 오른쪽으로 확실하게 우회!
        [850, 850],    # W6: 우회 후 하강 진입
        
        # --- 3. 우측 하단 파형 구간 (빨간 선의 우측 S자 지그재그) ---
        [1000, 700],   # W7: 바깥쪽(우측)으로 불룩 (웨이브 1)
        [850, 630],    # W8: 안쪽(좌측)으로 꺾임
        [970, 550],    # W9: 바깥쪽(우측)으로 불룩 (웨이브 2)
        [750, 450],    # W10: 안쪽 깊숙이 밑으로 쓸어담으며 턴
        [500, 500]     # W11: 출발 지점으로 완벽히 도착 완료!
    ]


    # Scenario 4: 완벽한 원 2개가 붙어있는 고밀도 8자(∞) 서킷
    spec4 = (
        [150, 750, radians(90)],  # 좌측 원의 맨 왼쪽(9시 방향)에서 위쪽(북쪽)을 향해 출발
        [
            # 각 원의 꼭대기/바닥 길목에 장애물 배치 (회피 기동용)
            [450, 1050, 40],   # 좌측 원의 상단 (12시 방향)
            [1050, 450, 40]    # 우측 원의 하단 (6시 방향)
        ],
    )
    
    waypoints4 = [
        [150, 750],    # W0: 시작 (좌측 원 9시 방향)
        [153, 789],
        [160, 828],
        [173, 865],
        [190, 900],
        ...
        [740, 828],
        [747, 789],
        [750, 750],    # 중앙 교차점 (우측 원 진입, 방향 꺾이지 않고 아래로 자연스럽게 진입)
        [753, 711],
        [760, 672],
        [773, 635],
        [790, 600],
    ...
    env = SimpleMazeGrid(
        ...
        dt=0.1,
        render_option=False,  
        spec=spec4,            # <--- Scenario 0, 1, 2, 3 중 선택 (spec, spec1...)
        waypoints=waypoints4,  # <--- Scenario 0, 1, 2, 3 중 선택 (waypoints, waypoints1...)
        sensor_range=100.0,          
    ...
    def visualize_comparison(env, results_dict):
        ...
        hard_zone_margin = getattr(env, "hard_zone", 2.0)
        for i, (oE, oN, oR) in enumerate(env.obstacles):
            # 👉 범례(Legend)에 표시하기 위해 첫 번째(i==0) 장애물에만 label 부여
            lbl_obs = 'Obstacle' if i == 0 else None
            circle = patches.Circle(
                (oE - offset, oN - offset), oR,
                edgecolor='black', facecolor='gray', alpha=0.5, zorder=1, label=lbl_obs
            ...
            # 👉 범례(Legend)에 표시하기 위해 첫 번째(i==0) 빨간 점선에만 label 부여
            lbl_hard = 'Hard Limit' if i == 0 else None
            hard_circle = patches.Circle(
                (oE - offset, oN - offset), oR + hard_zone_margin,
                edgecolor='red', facecolor='none', linewidth=1.5, linestyle=':', zorder=1, label=lbl_hard
        ...
        #     wps_x = wps_arr[:, 0] - offset
        #     wps_y = wps_arr[:, 1] - offset
        #     plt.plot(wps_x, wps_y, 'ko', markersize=8, label="Waypoints", zorder=2)
        #     for i in range(len(wps_arr)):
        #         if i == len(wps_arr) - 1: continue
        #         plt.text(wps_x[i] + 10, wps_y[i] + 10, f"W{i}", fontsize=16, color='black', weight='bold')

        # 배경: Global Path
        if env.global_path is not None and len(env.global_path) > 0:
            plt.plot(env.global_path[:, 0] - offset, env.global_path[:, 1] - offset,
                     color='cyan', linewidth=2, linestyle='--', label="Global Path", zorder=1)

        # 실험 결과 궤적 그리기
        for name, data in results_dict.items():
            traj = data['traj']
            plt.plot(traj[:, 0] - offset, traj[:, 1] - offset,
        ...
        # 👉 여기서 생성된 범례(Legend)에 Obstacle과 Hard Limit이 추가되어 나타납니다.
        plt.legend(loc='lower left', framealpha=0.8, facecolor='white', edgecolor='#cccccc', shadow=False, fontsize=15)
        plt.tight_layout()
        plt.show()

        # [Graph 2] Cross Track Error 비교
        plt.figure(figsize=(10, 6))
        for name, data in results_dict.items():
        ...
        plt.grid(True, linestyle="--", alpha=0.4)
        plt.legend(loc='upper right', framealpha=0.5, facecolor='white', edgecolor='#cccccc', shadow=False, fontsize=15)
        plt.tight_layout()
        plt.show()

        import pandas as pd

        # [Graph 3] Yaw Rate (w) 비교
        plt.figure(figsize=(10, 6))
        window_size = 10  
        ...
        for name, data in results_dict.items():
            if 'w' in data and len(data['w']) > 0:
                ...
                plt.plot(t_data, w_data, 
                         color=data['color'], linestyle=data['style'], linewidth=1.0, 
                         alpha=0.3, label=f"{name}_Raw")
                
                w_smooth = pd.Series(w_data).rolling(window=window_size, center=True, min_periods=1).mean()
                plt.plot(t_data, w_smooth, 
                         color=data['color'], linestyle=data['style'], linewidth=2.5, 
        ...
        plt.grid(True, linestyle="--", alpha=0.4)
        plt.legend(loc='lower left', framealpha=0.5, facecolor='white', edgecolor='#cccccc', shadow=False, fontsize=15)
        plt.tight_layout()
        plt.show()

    # ---------------------------------------------------------
    # 3. 실험 목록 (색상: Green, Blue, Red / 스타일: 실선)
    # ---------------------------------------------------------
    experiments = [
        # {
        #     "name": "Non-linear + APF",
