...
if __name__ == "__main__":
    ...
    def make_rolling_circle_track(center=(900.0, 700.0),
        ...
        """
        rolling circle 느낌:
        접선 방향 직선 진입 후 원을 크게 도는 형태
        """
        cx, cy = center
        th0 = np.deg2rad(entry_angle_deg)

        tangent = np.array([np.sin(th0), -np.cos(th0)], dtype=float)

        entry_pt = np.array([
            cx + radius * np.cos(th0),
            cy + radius * np.sin(th0)
        ], dtype=float)

        pts = []

        # entry line
        for d in np.linspace(line_len, 0.0, n_line, endpoint=False):
            p = entry_pt - d * tangent
            pts.append([p[0], p[1]])

        # main circle (clockwise)
        th_end = th0 - np.deg2rad(sweep_deg)
        for th in np.linspace(th0, th_end, n_circle):
            pts.append([
                cx + radius * np.cos(th),
                cy + radius * np.sin(th)
            ])
