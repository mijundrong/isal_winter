...
#         self.terminated_radius =20.0  # simple_maze_grid 기준, 원래 5.0
#         self.goal = False
#         self.visited_path = []
...
#         # 마지막 도착 시에만 종료 조건 완화
#         self.final_relaxed_radius = 45.0
#         self.final_phase_ratio = 0.97
#         self.final_wp_margin = 1
...
#         self.safety_zone = 35.0      # 기존 10.0 -> 50.0
#         self.hard_zone = 15.0        # 기존 3.5 -> 25.0
...
#         old_path_dist = self._compute_path_dist_from_current_window(old_pos[:2])
...
#             delta_d = (old_path_dist - new_path_dist) / max(self.v * self.dt, 1e-6)
...
#         if (not terminated) and self._check_goal_termination(new_pos[:2]):
#             reward += cfg["bonus_goal"]
#             self.goal = True
#             terminated = True

#             arrival_time = (self.steps + 1) * self.dt
...
#         if (not self.terminated) and self._check_goal_termination(new_pos[:2]):
#             self.goal = True
#             self.terminated = True
...
#     def _refresh_guidance_state(self):
#         self.update_reference_point()
#         self.obs_grid = self.compute_local_grids()
#         self.path_tracking()
#         self._update_current_goal_waypoint()

#     def _compute_path_dist_from_current_window(self, pos_xy):
#         if self.closest_path_point is None:
#             return None
#         pos_xy = np.asarray(pos_xy, dtype=float)
#         return float(np.linalg.norm(pos_xy - self.closest_path_point))

#     def _is_in_final_phase(self):
#         by_wp = False
#         if self.waypoint_states:
#             last_wp = len(self.waypoint_states) - 1
#             by_wp = self.current_wp_index >= max(0, last_wp - self.final_wp_margin)
...
#             and self.closest_path_idx is not None
#         ):
#             last_idx = len(self.global_path) - 1
#             th_idx = int(self.final_phase_ratio * last_idx)
#             by_path = self.closest_path_idx >= th_idx

#         return by_wp or by_path

#     def _check_goal_termination(self, pos_xy):
#         if self.final_goal_pos is None:
#             return False

#         cur_goal_dist = float(np.linalg.norm(np.asarray(pos_xy) - self.final_goal_pos[:2]))

#         if self.waypoint_states:
#             last_wp = len(self.waypoint_states) - 1
#             strict_ok = (self.current_wp_index >= last_wp) and (cur_goal_dist < self.terminated_radius)
#         else:
#             strict_ok = cur_goal_dist < self.terminated_radius

#         if strict_ok:
#             return True

#         if self._is_in_final_phase() and (cur_goal_dist < self.final_relaxed_radius):
#             return True
...
#                                   line_len=165.0,
#                                   exit_len=250.0,  # 추가: 교차점 지난 뒤 바깥으로 빠지는 길이
#                                   n_line=14,
#                                   n_circle=120,
#                                   n_exit=12,  # 추가: exit 직선 점 개수
#                                   clockwise=True):
#         """
#         rolling circle:
#         - 좌하단 직선 진입
#         - 원 위 한 점에서 접선 방향으로 진입
#         - 정확히 한 바퀴 선회
#         - 교차점을 지난 뒤 직선으로 충분히 이탈
#         - 모든 점에 heading 포함
#         """
#         cx, cy = center
...
#         # 원 접선 heading
#         if clockwise:
#             psi_tangent = th_cross - np.pi / 2.0
#             theta_end = th_cross - 2.0 * np.pi
#         else:
#             psi_tangent = th_cross + np.pi / 2.0
#             theta_end = th_cross + 2.0 * np.pi

#         psi_tangent = np.arctan2(np.sin(psi_tangent), np.cos(psi_tangent))
...
#         p_exit_end = p_cross + exit_len * line_dir
...
#         # 2) 원 한 바퀴
#         thetas = np.linspace(th_cross, theta_end, n_circle + 1)
#         for th in thetas:
...
#         # 3) 교차점 통과 후 직선 이탈
#         # 원 마지막 점이 이미 p_cross 이므로 endpoint 중복 방지
#         for t in np.linspace(0.0, 1.0, n_exit + 1)[1:]:
#             p = (1.0 - t) * p_cross + t * p_exit_end
#             pts.append([
#                 round(float(p[0]), 1),
#                 round(float(p[1]), 1),
#                 float(psi_tangent)
...
#     spec2 = (
#         [stadium_pts[0][0], stadium_pts[0][1], stadium_heading],None,
#         # [
...
#     spec3 = (
#         [rolling_pts[0][0], rolling_pts[0][1], rolling_pts[0][2]], None,
#         # [
#         #     [900, 450, 40]
#         # ],
#     )
...
#     # ---------------------------------------------------------
# # ---------------------------------------------------------
#     # 3. 실험 목록 (기존 RL 모델에 APF 모델 추가)
#     # ---------------------------------------------------------
#     experiments = [
#         {
#             "name": "Nonlinear Control (APF)", # 그래프 범례에 표시될 이름
#             "type": "apf",                     # simulation 로직 분기를 위한 타입
#             "path": None,                      # 모델 로드가 필요 없으므로 None
#             "color": "black",                  # 그래프 색상 (원하는 색으로 변경 가능)
#             "style": "--"                      # 점선으로 표시하여 RL 모델과 구분
#         },
#         {
#             "name": "Model 1 (track+eta)",
#             "type": "sac",
#             "path": "best_model",
#             "color": "tab:green",
#             "style": "-"
...
#             "name": "Model 2 (track)",
#             "type": "sac",
#             "path": "sac_maze_checkpoint_540000",
#             "color": "tab:orange",
#             "style": "-"
#         },
#         {
...
#     # =========================================================
#     # 5. 정량적 지표 (비행 시간 & Max Error) 분석 및 출력
#     # =========================================================
#     print("\n===== 정량적 지표 분석 결과 =====")
...
#                 total_time = data['time'][-1] if len(data['time']) > 0 else 0.0
#                 max_err = np.max(err_data) if len(err_data) > 0 else float('nan')
#                 mean_err = np.mean(err_data) if len(err_data) > 0 else float('nan')
#                 p95_err = np.percentile(err_data, 95) if len(err_data) > 0 else float('nan')
